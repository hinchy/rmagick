...
Image_convolve(VALUE self, VALUE order_arg, VALUE kernel_arg)
{
    ...
    {
        kernel_info->values[x] = NUM2DBL(rb_ary_entry(kernel_arg, (long)x));
    }
...
Image_convolve_channel(int argc, VALUE *argv, VALUE self)
{
    ...
    for (x = 0; x < order*order; x++)
    {
        kernel_info->values[x] = NUM2DBL(rb_ary_entry(ary, (long)x));
    }
...
Image_sharpen_channel(int argc, VALUE *argv, VALUE self)
{
    ...
    exception = AcquireExceptionInfo();
    SetImageChannelMask(new_image, channels);
    (void) SharpenImage(new_image, radius, sigma, exception);
